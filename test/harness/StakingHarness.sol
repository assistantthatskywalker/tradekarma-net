...
/**
 * @notice Exposes Staking's internal pure math so the geometric mean can be
 *         fuzzed over the full uint256 range without needing token balances
 *         that large to exist.
 */
contract StakingHarness is Staking {
    constructor(address _krune, address _kdex, address _kshrd, address _treasury, address owner_)
        Staking(_krune, _kdex, _kshrd, _treasury, owner_)
    {}
